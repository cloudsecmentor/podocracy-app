...
let instructionsProjectId = null;
...
function instructionsPanel(data) {
  const generated = data.autogenerated_custom_instructions || "";
  const used = data.effective_used || "";
  return `
    <div class="editor instructions">
      <h3>Instructions</h3>
      <label class="instructions-label">
        Your instructions
        <textarea id="instructions-editor" spellcheck="false" rows="5">${escapeHtml(data.custom_instructions || "")}</textarea>
      </label>
      <div class="editor-actions">
        <button type="button" id="save-instructions" class="secondary">Save</button>
        <span class="muted">Saved to the project config and applied on the next run.</span>
      </div>
      <h4>Autogenerated ${data.autogenerate_enabled ? "" : '<span class="muted">(generation is off)</span>'}</h4>
      ${generated
        ? `<pre class="instructions-generated">${escapeHtml(generated)}</pre>
           <div class="editor-actions">
             <button type="button" id="copy-instructions" class="secondary">Copy into my instructions</button>
           </div>`
        : `<p class="muted">Nothing generated yet. The customize stage adds suggestions here when it runs.</p>`}
      ${used
        ? `<details class="instructions-used">
             <summary>Text actually sent to the improve stage</summary>
             <pre>${escapeHtml(used)}</pre>
           </details>`
        : ""}
    </div>`;
}

async function saveInstructions(projectId) {
  const editor = detailEl.querySelector("#instructions-editor");
  if (!editor) return;
  const button = detailEl.querySelector("#save-instructions");
  ...
  try {
    await api(`/api/projects/${projectId}/instructions`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ custom_instructions: editor.value }),
    });
    if (instructions) instructions.custom_instructions = editor.value;
    instructionsDirty = false;
    setActionMessage("Instructions saved. Requeue the project to apply them.", "ok");
    await loadProjects();
  } catch (error) {
...
async function renderDetail(project) {
  ...
  if (needsFullBuild && !(instructionsDirty && instructionsProjectId === project.id)) {
    try {
      instructions = await api(`/api/projects/${project.id}/instructions`);
      instructionsProjectId = project.id;
      instructionsDirty = false;
    } catch {
      instructions = null;
  ...
  if (needsFullBuild) {
    ...
    const instructionsEditor = detailEl.querySelector("#instructions-editor");
    if (instructionsEditor) {
      instructionsEditor.addEventListener("input", () => {
        if (instructions) instructions.custom_instructions = instructionsEditor.value;
        instructionsProjectId = project.id;
        instructionsDirty = true;
      });
    }
    const saveInstructionsButton = detailEl.querySelector("#save-instructions");
    if (saveInstructionsButton) {
      saveInstructionsButton.addEventListener("click", () => {
        void saveInstructions(project.id);
      });
    }
    const copyInstructionsButton = detailEl.querySelector("#copy-instructions");
    if (copyInstructionsButton) {
      copyInstructionsButton.addEventListener("click", () => {
        if (!instructionsEditor || !instructions) return;
        const generated = instructions.autogenerated_custom_instructions || "";
        const existing = instructionsEditor.value.trim();
        instructionsEditor.value = existing ? `${existing}\n--------------\n${generated}` : generated;
        instructions.custom_instructions = instructionsEditor.value;
        instructionsDirty = true;
