...
def sound_prep(soundFile):
    ...
    sound.export(soundFile, format ="wav")
...
def main(args):
    if len(args) != 3:
        sys.stderr.write(
            'Usage: silenceremove.py <aggressiveness> <pathToWavFile> <pathToNonSilencedWavFile>\n')
        sys.exit(1)

    resPrep = sound_prep(args[1])
    print ('File ' + args[1] + ' prep: ' + resPrep)
    ...
    write_wave(args[2], joinedaudio, sample_rate)
