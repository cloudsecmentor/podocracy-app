...
def main(path):
    ...
    try:
        user_instructions = get_params("custom_instructions", path)
    except:
        user_instructions = ""
    try:
        autogenerated_instructions = get_params("autogenerated_custom_instructions", path)
    except:
        autogenerated_instructions = ""

    custom_instructions = compose_custom_instructions(user_instructions, autogenerated_instructions)
    logging.info(
        f"Using custom instructions: user [{len(str(user_instructions or ''))} chars] "
        f"autogenerated [{len(str(autogenerated_instructions or ''))} chars]"
    )

    # Record exactly what was sent to the model, so the portal can show the effective text.
